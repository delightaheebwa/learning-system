...
GATE:fact_check — verify each numbered claim against ALL listed sources (source_url and/or source_urls — Rohit plus external refs) and your own knowledge. Be strict on mechanism claims, lenient on phrasing. If the sources disagree with each other, say so in the explanation. If the sources are silent, check your own knowledge; if unsure, mark UNVERIFIED rather than guessing. Output ONLY valid JSON:
