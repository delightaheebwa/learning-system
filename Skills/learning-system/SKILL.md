...
- **"lesson" / "continue" → next curriculum lesson** — `Skills/learning-teach/SKILL.md` (Rohit roadmap order; Mission 0 done, P1 L07 done — resume P1 L08).
...
1. Load `Learning System/MISSION.md` + `Learning System/CURRICULUM.md` to determine the next lesson (or the requested topic). Position (2026-09-08): Mission 0 Catch-Up **done**; Phase 1 L07 (Bayes) **done**; **resume Phase 1 L08 — Optimization (paused Checkpoint 2/5)**. If resuming (`Lessons/` file exists), the lesson file + last `Sessions/` note are the source of truth — no Scout digest needed.
...
Trigger: "review" (AIEFS). `swe` is archived — redirect to AIEFS. **Run this flow on the Clerk preset** (switch to Clerk first if you are on another preset).
...
6. After each answer: grade pass/fail, then verify the grade with ONE **foreground** `GATE:grade_audit` envelope via `delegate_task` (`background:false`) before presenting it — batch per concept (`{"gate":"grade_audit","concept","question","learner_answer","claimed_verdict":"pass|fail","source_excerpt"}` with a Concept Note / Lesson / Wiki excerpt fetched via `bundle`; include `feynman_transcript` for `concept`/`design` explain-backs). **Grade turns take `grade_audit` ONLY — never `fact_check` for a grade; a `fact_check` receipt does not satisfy a grade turn and the gate will block it.** If your grade turn also teaches new claims, keep the turn grade-only (teach claims in a separate turn with its own `fact_check`); the gate checks grade turns for `grade_audit` alone. The gate Pipe (`gate_pipe.py`) validates the receipt before the grade renders (retry cap 2/turn → `⛔ Withheld`). If the verifier disagrees (`correct_verdict` differs), use its verdict. Then record via `python3 scripts/ops.py attempt "Concept" pass|fail [feynman_pass|feynman_fail]` — this updates Attempts.json (recency-weighted mastery 0–1, confidence cap {1:0.5, 2:0.8}, interval_index +1 on pass / +2 on 2 consecutive passes / -1 on fail, next_review). On fail also append row to `🧯 Mistakes.md` with error_type (`structural|deviation|application|metacognitive`) and self-attribution; on next correct recall bump Retries, after 2 consecutive correct mark `graduated`. For `concept`/`design` types, elicit Feynman explain-back (own words, when/why, nearest-neighbor distinction, one example) and pass `feynman_pass`/`feynman_fail` — score shown but **not blocking** (advisory mode). Then write `Reviews/Review — [Concept] — [Date].md` and update Active Concepts `last_reviewed`/`next_review` (from Attempts.json) / `Last Q Type`.
...
9. **Session-close audit (`GATE:review_session`) — before the final summary renders.** After the Review note(s), session note, Active Concepts rows, Mistakes rows, and Attempts.json transitions are written, dispatch ONE **foreground** `GATE:review_session` envelope via `delegate_task` on the exact content written (generation-to-emission — never a summary). The Pipe (`gate_pipe.py`) checks that each `written_files[].path` exists and matches the reviewed text. Verify: each written verdict matches its `grade_audit` `correct_verdict`; `next_review`/`last_reviewed`/`Last Q Type` match the Attempts interval logic; fail ⇒ Mistakes row with `error_type`; graduation only after 2 consecutive correct; session tallies match the transcript. Scope is fenced — MISSION/CURRICULUM/Profile/Learner History/wiki drift is `context_notes`, never an `issue`. Fix high/medium issues and re-dispatch `pass_number: 2`; **hard cap 2 cycles**. After cycle 2 the Pipe renders the summary with a `⚠️ REVIEW FLAGS SURFACED` banner (never a third pass, never a withhold). Save the verdict to `Learning System/Reviews/Session Audits/<concepts>-pass<N>-<date>.json` and show the result. See `learning-review` for the envelope. `swe` archived concepts use the same flow.
...
5. Refresh learner context — run `python3 scripts/learner_history.py` (regenerates `Core/Learner History.md` from Attempts/Mistakes/Reviews) and include it in the commit.
...
- **Review-session close (Clerk, review flow):** the end-of-review summary requires ONE foreground `GATE:review_session` envelope auditing the exact writes (Review notes, session note, touched Active Concepts / Mistakes / Attempts rows) against the transcript + grade verdicts; scope is fenced (state drift → `context_notes`) and the Pipe renders with a `⚠️ REVIEW FLAGS SURFACED` banner on ISSUES after the 2-cycle cap rather than withholding.
