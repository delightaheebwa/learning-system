...
- **Review gate (ingest step 7):** call the `review_gate` tool, which invokes a SECOND model (`ox-alpha-free`) as the independent reviewer. Pass the source URL, concept names, and the wiki content (read via the terminal). See `OPENWEBUI.md` at the repo root for setup.
- **Teaching verification:** call the `fact_check` tool (`ox-alpha-free`) for load-bearing claims before presenting them.
