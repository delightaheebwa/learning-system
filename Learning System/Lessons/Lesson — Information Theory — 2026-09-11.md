...
Status: **paused at Checkpoint 4/6 (2026-09-14)** — resume at CP5 (mutual information)

## Resume from
Resume at **CP5 — mutual information** (fresh checkpoint: idea → pause → practice → grade). Open with a recall warm-up on CP4 (KL) — last completed checkpoint — before teaching CP5. 2026-09-14 outcome: CP4 practiced and graded pass: P1 (a/b/c = 1.000 / 0.469 / 0.531 bits) + P2 (C), all correct with tag sure — grade-audit PASS agreeing; attempts logged: "KL Divergence" pass (interval_index 1, next_review 2026-09-21) and "Cross-Entropy from NLL" pass (next_review 2026-09-21). Learner paused at CP4 Pause #2 and declined the exit ticket (CP4 practice just graded — accepted). Convention note (for CP5+): this lesson uses Olah-style H(P,Q) with weights from truth Q, surprises from model P, so the floor is H(Q); Rohit/ML convention flips the letters (P = data → H(P,Q) ≥ H(P)). Anchor phrase: "the entropy term is always the data's entropy". The learner asked about the letter convention on 2026-09-12 and got the verified reconciliation.
