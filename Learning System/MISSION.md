...
- **Position:** Phase 1 L07 — Bayes' Theorem — **done** (2026-09-05). Phase 1 L08 — Optimization (Gradient Descent Family) — **done** (2026-09-10; cumulative quiz + Feynman pass). Phase 1 L09 — Information Theory (Entropy, KL Divergence) — **paused mid-Checkpoint 5/6 (2026-09-18)**: CP1–CP4 banked (CP4 re-sealed — the 2026-09-16 warm-up went 4/4 under fresh numbers, so the re-seal now holds across two sessions; attempts 'KL Divergence' interval_index 3, next_review 2026-10-19); CP5 delivered so far: the idea + the four-form breakdown + the KL-connection walkthrough (I(X;Y) = KL(p(x,y) ∥ p(x)p(y)) walked step by step, numerically checked at 0.531 bits on the running joint, with the bars / set-strip intuition confirmed exact) + the nonnegativity re-digest (the rebate/cost reading of the per-term signs, Gibbs' inequality Σ Q_i·log2(Q_i/P_i) ≥ 0, the one-line Jensen proof with the deliberate numerator reversal, the Jensen-vs-Gibbs division of labour) — exit-ticketed 3/3. Still open inside CP5: the E1 KL-form slip RECURRED in the 2026-09-18 warm-up (answered "X (the truth) then Y" — variables instead of distributions, grade-audit FAIL), the fresh I(X;Y) computation on a NEW joint has still never been tested, and there is no Feynman yet. Resume chain: (1) CP5 practice on a fresh joint, (2) V(X,Y) = H(X,Y) − I(X,Y) consolidation, (3) CP6 (perplexity + bits/nats + label smoothing), (4) Feynman explain-back + final cumulative quiz, then the lesson-end handoff. See `CURRICULUM.md`.
