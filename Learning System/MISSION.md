...
- **Position:** Phase 1 L07 — Bayes' Theorem — **done** (2026-09-05). Phase 1 L08 — Optimization (Gradient Descent Family) — **done** (2026-09-10; cumulative quiz + Feynman pass). Phase 1 L09 — Information Theory (Entropy, KL Divergence) — **paused mid-Checkpoint 5/6 (2026-09-16)**: CP1–CP4 banked (CP4 re-sealed — the 2026-09-16 warm-up went 4/4 under fresh numbers, so the re-seal now holds across two sessions; attempts 'KL Divergence' interval_index 3, next_review 2026-10-17); CP5 idea + the four-form breakdown + the KL-connection walkthrough delivered (I(X;Y) = KL(p(x,y) ∥ p(x)p(y)) walked step by step, numerically checked at 0.531 bits on the running joint, with the bars / set-strip intuition confirmed exact) — the two CP5 repairs (nonnegativity re-digest, E1 KL-form slip), CP5 practice on a fresh joint and the Feynman are still pending, so resume **inside CP5** at those repairs, not at the idea. See `CURRICULUM.md`.
